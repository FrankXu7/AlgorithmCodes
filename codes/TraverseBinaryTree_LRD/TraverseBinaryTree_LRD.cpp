 ...
 * 
 * 方法三：
 * 双栈法的逻辑相对简单，但是用到了两个栈，而且因为得到的是后续遍历的反序序列，还需要将序列倒置一下。
 * 参考中序遍历的做法，中序遍历只用一个栈，在弹出栈顶元素的时候，如果有右节点，则从右节点开始，
 * 依次压入左节点到叶子节点为止，这样序列就符合中序遍历“左根右”的特性。这里可以得出，如果栈顶元素在弹出前，
 * 提前判断到存在右节点，先不弹出栈顶元素，而直接从右节点开始，依次压入左节点到叶子节点为止。
 * 这样构建的序列就符合后续遍历“左右根”的特性。大致过程：
 ...
 * 
 * 方法四：
 * 采用Morris遍历，二叉树的Morris后续遍历比前序和中序更复杂一点，假如某个节点A的左侧分支的最右节点的右指针指向A，
 * 当遍历到这个最右侧节点时，此时已经无法直接回到这个最右侧节点的父节点了，也就失去了“左右根”中“根”的指向。
 * 仔细想想，当遍历一圈回到A节点时，之前那个找不到的最右侧节点的父节点，其实就在A节点左侧分支的最右侧的路径上，
 * 显然第一个这种路径就是二叉树最左侧的节点。而二叉树的后续遍历，符合这么一个规律：从树的最左侧节点所在的最右侧路径开始，
 * 逐层网上，逆序遍历，就是二叉树的后续遍历了，比如题目中输入的二叉树，路径从左往右，节点从上到下，第一条路径是：1，
 * 第二条路径是：2、3，第三条路径是：5，第四条路径是：4、6、7；依据路径顺序逆序输出，刚好就是二叉树的后续遍历。
 * 算法大致步骤：
 * （1）CurNode.left 不为空，找到以 CurNode.left 为根节点的最右侧节点 MostRightNode，
 *		I.  MostRightNode.right 为空，借用其指针变量，指向当前遍历节点，MostRightNode.right = CurNode，这样保证后面
 *			遍历的节点在左右移动的时候还能够回到根节点。然后当前遍历节点左移，CurNode = CurNode.left；
 *		II. MostRightNode.right 不为空且等于 CurNode，此时就回到了第 I 种情况中的 CurNode，以 CurNode.left 所在的最右侧路径
 *			为分界线，其左侧所有节点均已输出，该路径需要【逆序】输出以保证“右根”的特性，因为 MostRightNode.right 记录了第 I 种
 *			情况中的 CurNode，所以遍历中止条件就是回到了 CurNode，然后需要特别注意要将 MostRightNode 置 nullptr，因为算法
 *			只是借用了空指针变量，不能改变树的结构。
 * （2）CurNode.left 直接右移，在步骤（1）的第 II 种情况中，来处理 CurNode.left 所在最右侧路径的遍历输出问题。值得注意的是，
 *		这个步骤也保证了步骤（1）的第 II 种情况中，不会出现 MostRightNode.right 不为空且不等于 CurNode 的情况。
 * 需要特别注意的是，针对二叉树最后一条路径（即最右侧路径）在遍历结束后这条路径还未输出，需要以根节点为起始节点，逆序输出该路径节点。
 *
 * 【解题分析】
 * 方法一：
 * 时间复杂度：O(N)
