 ...
 * 【解题思路】
 * 方法一：
 * 最先想到的就是遍历一个数组元素，并判断其在另一个数组的存在性，不过这样会导致较高的时间复杂度，
 * 可以牺牲一定的空间复杂度降低之。将俩数组中长度较短的数组元素存入一个无序的set集合，然后对长度较长的
 * 数组进行遍历，判断元素在set集合中的存在性。因为题意要求最终数组元素唯一，所以还需要一个辅助的无序
 * set集合存储最终结果，在将相交元素推入最终数组前，先判断一下辅助的set集合内是否存在该元素，存在则表示
 * 之前已经存储过了，不要重复存储。
 * 
 * 方法二：
 * 双指针法，如果两个数组是有序的情况下，可同时遍历俩数组：
 * （1）当两个下标指向的元素不同时，将更小的值所对应的下标自加；
 * （2）当两个下标指向元素相同时，则推入最终数组，然后俩下标都自加，这里要注意，推入之前，需要判断最终数组
 *		末尾元素是否和即将要推入的元素相同，若相同为保证元素唯一则不推入。因为俩数组已经有序，所以如果存在
 *		多个相同的相交元素，则它们一定是相邻的，故可以判断最终数组末尾元素是否和即将推入元素相同来保证唯一性。
 * 
 * 【解题分析】
 * 方法一：
 * 时间复杂度：O(N) 实际为O(M + N)，创建集合与元素比较分别需要一次遍历 
 * 空间复杂度：O(N) 实际为O(2N)，N为长度更短的数组的元素数量，只有完全相交且元素唯一的情况下，才会达到O(2N) 
 * 方法二：
