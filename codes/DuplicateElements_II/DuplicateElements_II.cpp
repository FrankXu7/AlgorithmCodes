/**************************************************************************************************
 * 【题目描述】
 * 判断一个整数数组是否存在相同元素，同时给定一个整数 idxDifference，如果这几个相同元素的下标的差值，存
 * 在不超过给定的整数 idxDifference，则返回true，反之返回false；
 * 
 * 【输入】
 ...
 * 
 * 方法二：
 * 因为给定了一个最大下标差值，可以利用“窗口滑动”思想，使用无序的set集合，如果将不重复的数组元素放入无序
 * set集合，依据题意要求，可以得出无序set集合内的元素数量，必然不会超过给定的最大下标差值，所以，当放入一
 * 个元素导致无序set集合容量超过给定的最大下标差值时，需要将首元素移除，大致过程：
 * （1）判断当前遍历的元素如果存在于无序set集合，表示集合中已经存在与当前遍历元素数值相同的元素，且下标差
 *      值在给定范围内，直接返回true，结束算法；
 * （2）如果当前遍历元素在无序set集合不存在，则将元素放入集合，放入后需要进一步判断容器元素数量是否超过最
 *      大下标差值，若超过，需要将首元素移除。
 * 
 * 
 * 【解题分析】
