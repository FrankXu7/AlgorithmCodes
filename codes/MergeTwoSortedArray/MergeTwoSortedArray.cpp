 ...
 * 对数组1扩容，数组1扩容后容量 = 数组1扩容前容量 + 数组2容量；这里需要保存扩容前数组1数据的最大下标，用0
 * 以之后的合并比较；从扩容后的数组1的末尾开始，依次反序遍历数组2，和扩容前数组1数据的最大下标；将数据从
 * 大到小，依次从扩容后的数组1末尾插入。
 * 
 * 【解题分析】
 * 时间复杂度：O(N)，这里的N指的是两个数组长度之和，需要比较两个数组所有元素
 * 空间复杂度：O(1)
 * 
 * @author FrankX
 * @date 2021-04-21
...
void MergeTwoSortedArray(vector<T>& beMergeArr, vector<T>& mergeArr)
{
	...
	for (int i = beMergeSize - 1, k = mergeArr.size() - 1; i >= 0 || k >= 0;)
	{
		...
		else continue; // 这里break其实也可以，continue是为了保证for循环的结束条件判断上统一 

		++itr;
	}

	// 置换掉合并数组内的数据，交换给一个局部容器，函数返回时该容器会被析构 
