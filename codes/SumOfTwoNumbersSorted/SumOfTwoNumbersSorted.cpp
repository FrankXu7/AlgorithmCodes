/**************************************************************************************************
 * 【题目描述】
 * 计算一个升序数组中是否存在两个元素之和会等于给定的数，并找出它们的下标，假设数组中只有一组答案，且同数
 * 组元素只能使用一次，返回的下标从 1 开始计数。
 *
 * 【输入】
 ...
 * 【解题思路】
 * 方法一：
 * 最简单直观的方式就是暴力法，两层嵌套循环相加后与目标值比较。但需要注意到的是，给定数组的特殊性：是一个
 * 升序数组。所以可以采用二分查找，减少遍历次数。当然还是需要两层嵌套循环，第一层循环确定一个数字，通过目
 * 标数字与确定的数字的差值，可得出满足题意所需要的数字。这个数字会在第二层遍历中用二分法在升序数组中得出。
 * 大致步骤：
 ...
 *
 * 方法二：
 * 因为数组是升序的，也可以使用双指针法，进一步降低时间复杂度。将左右指针分别指向升序数组头尾数字，左右指
 * 针指向数据之和，如果小于目标数字，则左指针++，若大于目标数字，则右指针--，逐渐逼近目标数字。当左指针大
 * 于右指针时还没找到，即终止循环，结束算法。
 * 
 * 【解题分析】
 * 方法一：
