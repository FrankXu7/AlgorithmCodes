 ...
 *
 * 【解题思路】
 * 以最长的字符串长度作为遍历边界，反序遍历两个字符串。
 * 对同下标的两个字符代表的数字，与进位的值相加：c1 + c2 + carry，依题可知这三个数只可能是0或1，
 * 它们的和（这里是在十进制规则下相加的和）的个位数，即为结果字符串对应下标的字符，
 * 有偶数个1，则和的个位数是0，有奇数个1，则和的个位数是1；
 * 需要注意进位的情况，若这三个数之和大于2，代表有进位。
 * 针对字符串中非“0”和“1”的字符，统一用“0”填充。
 * 
 * 【解题分析】
 * 时间复杂度：O(N)
 * 空间复杂度：O(1)
