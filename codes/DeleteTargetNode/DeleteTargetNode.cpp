 ...
 *
 * 【解题思路】
 * 删除链表中节点，最先会想到的就是正儿八经的遍历一遍，找到数据与指定值相等的节点，移除之。但题中有很多限
 * 制条件，比如参数只能是被删除节点，故只能从被删除节点开始访问到下一个节点，所以，可以将下一个节点的值赋
 * 值到被删除节点，并且将被删除节点指向下一个节点的再下一个节点，这样被删除节点的数据被替换为了它的下一个
 * 节点的数据，且指向也被正确设置，就相当于是删除了指定节点。
 * 
 * 此法不能删除最后一个节点，因传入的被删除节点无法往回访问，但题中限制了不能删除尾节点且各节点数据值唯一。
 * 
 * 【解题分析】
 * 时间复杂度：O(1)
 * 空间复杂度：O(1)
