 ...
 * 【解题思路】
 * DFS遍历法：
 * 使用递归实现深度遍历二叉树，记录从根节点到当前节点的路径字符串，并作为参数传到下一次递归中去，当递归到
 * 叶子节点时，将当前节点追加到路径字符串中，并推入作为参数传入的容器中，容器必须是左值引用。
 * 
 * BFS遍历法：
 * 使用迭代实现广度遍历二叉树，用两个双端队列，一个记录节点，另一个记录从根节点到该节点的路径字符串，正常
 * 情况下两个队列元素数量是相等的。在一次遍历中，同时出队两个队列的首元素，如果出队的节点不是叶子节点，则
 * 将其左右孩子节点入队到节点队列，并依据左右孩子的数据值和当前节点的路径字符串，组合新的左右孩子路径字符
 * 串，入队到路径字符串队列；若本次遍历出队的节点是叶子节点，则将对应出队的路径字符串推入最终需要返回的路
 * 径容器。
 * 
 * 【解题分析】
 * DFS遍历：
