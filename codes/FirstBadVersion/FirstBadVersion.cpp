/**************************************************************************************************
 * 【题目描述】
 * 假设一个产品经理，目前正在带领一个团队开发新产品，但不幸的是，产品没有通过质量检测。由于每个版本都是基
 * 于之前的版本开发的，所以从某一个错误的版本之后的所有本吧，都认为是错误的。
 * 
 * 假设有 N 个版本，[1, N]，需要找出导致后续所有版本出错的第一个错误的版本。
 * 
 * 可通过一个 bool IsBadVersion(versionNum) 接口来判断版本号 versionNum 是否在测试中出错，应该尽可能减
 * 少算法API的调用。
 * 
 *
 * 【输入】
 ...
 *
 * 【解题思路】
 * 本题可以理解为，在 [1, N] 中，前半部分的版本的测试结果都是正确的，后半部分版本的测试结果都是错误的，要
 * 找到首个错误版本。且假定已经有一个接口 bool IsBadVersion(versionNum) 来判断指定的版本号是否在测试中出
 * 错。可以使用二分法查找，以减少核心算子执行次数，最开始从数组首尾元素开始，作为边界，计算中间元素，判断
 * 中间元素是否在测试中出错，如果是，则表明首个出错版本在中间元素左侧，反之亦然，然后缩紧计算边界，重新计
 * 算中间元素，直到边界重合，即找到首个测试出错的版本。
 *
 * 【解题分析】
 * 时间复杂度：O(logN)
 * 空间复杂度：O(1)
...
/**
 * @brief 辅助函数，用以判断当前传入的版本号，是否在测试中错误，为了接近实际业务需求，这里假定版本号大于
 * 等于某个局部常量值，即为错误版本号，其实这个值也是首个错误版本号，但实际业务中对外部是未知的，只是用于
 * 模拟。
