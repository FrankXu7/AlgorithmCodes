/**************************************************************************************************
 * 【题目描述】
 * 给定一个整数n属于[0, 33]，返回从0开始算起的第n行杨辉三角形。在数学上，杨辉三角形（也叫帕斯卡三角形）具
 * 有如下性质：
 * （1）每行数字左右对称，且从中间开始往两边逐渐减小到1，首尾都是1；
 * （2）从0开始算的第n行满足d=1的等差数列，前n行有 n(n + 1) / 2 个元素；
 * （3）从0开始算第n行有 n + 1 个元素；
 * （4）某个元素必为上一行左右两个元素之和，上一行元素不存在当作0处理，即row行column列元素为row-1行中的
 *      column-1列和column列元素之和；
 ...
 *
 * 【解题思路】
 * 考虑上述第四点性质，可得等式：Data[row][column] = Data[row][column-1] + Data[row-1][column]; 更大规
 * 模的问题可由有限的更小规模问题得出答案，类似动态规划的思想，大致过程：
 * （1）初始化每行元素，每行元素的个数是已知的；
 * （2）每行的首尾元素可直接赋值为1；
 * （3）遍历每行除首尾的元素，其值为上一行对应下标和前一个小标所在元素之和，这里因为步骤（2）已经给首尾元
 *      素赋值，所以遍历范围为[1, column-1]，这样可以避免边界的判断问题；
 * 
 * 【解题分析】
 * 时间复杂度：O(N^2)
 * 空间复杂度：O(N) 这里的N包含当前行生成的数据，和上一行生成的数据之和，N会随着问题规模，即要求的行数的
 *                  增大而增加。
 * 
 * @author FrankX
 * @date 2021-06-03
