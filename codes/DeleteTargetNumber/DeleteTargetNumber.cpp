 ...
 *
 * 【解题思路】
 * 利用“双指针”，即数组头尾下标，当左下标对应元素等于目标数字时，用右下标的数字替换之，右下标--，此时需要
 * 注意的是，若发生了右下标元素替换左下标元素，则左下标不能++，因为还需要下次遍历判断替换元素是否等于目标
 * 元素。当左下标小于等于右下标时，标识结束，此时数组中等于目标数字的元素都被右下标所指向过的元素替换，返
 * 回（左下标+1）即为有效元素个数。
 * 
 * 【解题分析】
 * 时间复杂度：O(N)
