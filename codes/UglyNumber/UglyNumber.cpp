 ...
 *
 * 【解题思路】
 * 依据题意，首先0和负数都不是丑数，所以数据结构采用 unsigned int，其次题设数字1是丑数，所以算法开始需要
 * 针对0和1进行分支处理。如果一个数是丑数，将它对2取余，若结果为0则再整除2，然后继续上述操作直到对2取余不为0，
 * 再之后，继续对3和5进行同样的处理，因为都有取余判断，所以丑数经过这一系列计算后结果必为1，否则就不是丑数。
 *
 *
 * 【解题分析】
