/**************************************************************************************************
 * 【题目描述】
 * 给定一个股票价格数组，它的第idx个元素表示第idx天股票的价格。选择其中第idx天买入股票，并且在未来某一个
 * 不是买入天的日子卖出，只能进行一次买入卖出，设计算法计算可获得的最大利润。如果买入后卖出必亏，即价格数
 * 组为一个降序数组，表示无利润，返回0。
 * 
 * 【输入】
 ...
 *
 * 【解题思路】
 * 计算第idx天卖出获得的最大收益，肯定希望之前买入的是最低价，所以需要记录第idx天之前的最低买入价；同时，
 * 第idx天之前的最大收益，并不代表是总的最大收益，也需要记录第idx天之前的最大收益便于后续遍历比较。利用动
 * 态规划思想：第idx天的最大收益 = Max( 记录的最大收益，第idx天售出价格-记录的最低买入价格 )。
 * 
 * 具体实现步骤：
