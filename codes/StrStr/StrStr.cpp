 ...
 *
 * 【解题思路】
 * 遍历搜索字符串，如果发现某个字符和子字符串首字符相同，则标识开始匹配，当字符串搜索完后，即得到结果；
 * 如果匹配搜索发现有字符不相等，需要重置搜索数据（重置记录的搜索起始下标和搜索下标）；
 * 特殊情况：当首字符相等，开始匹配时，如果子字符串长度，大于搜索字符串剩余未比较字符数，返回-1，标识未匹配到。
 *
 * 【解题分析】
 * 时间复杂度：O(N)
 * 空间复杂度：O(1)
