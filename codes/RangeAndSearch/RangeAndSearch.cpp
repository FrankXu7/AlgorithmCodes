 ...
 * 赋值给第 i 个元素。即为下标区间 [0, i] 的元素之和，满足表达式：sums[i] = nums[i] + sums[i-1]，
 * 简而言之，通过这一通计算，数组中下标为 i 的元素，表示下标区间[0, i]的元素之和。
 * 
 * 这样就建立了一个状态转移方程：sums[i, j] = nums[j] + nums[i-1]，求下标区间 [i, j] 的和被转换为求
 * nums[j] 与 nums[i-1] 的差值，题意要求是闭区间，所以减去的是 nums[i-1]，就是下标小于 i 的元素之和。
 * 
 ...
 * 空间复杂度：O(N) 需要存储数组元素
 * 方法二：
 * 时间复杂度：O(1) 初始时，需要构造一个元素为前N项和的数组，复杂度为O(N)，之后求区间和只需要计算一个差值即可
