/**************************************************************************************************
 * 【题目描述】
 * 给定一个数组 nums，将数组中所有的 0 移动到数组末尾，同时保持非零元素的相对顺序不变。
 * 必须在原数组上操作，不能使用辅助空间，且需要尽量减少操作次数。
 *
 * 【输入】
 ...
 *
 * 【解题思路】
 * 可以使用快慢指针法，将快指针指向元素替换到慢指针指向元素，如果快指针指向的是0，则快直接指针后移一位，
 * 不进行替换。当快指针指向边界时，慢指针指向的元素左侧是该数组全部非0元素，所以将慢指针右侧都直接赋值为0即可。
 *
 * 【解题分析】
 * 时间复杂度：O(N)
 * 空间复杂度：O(1)
