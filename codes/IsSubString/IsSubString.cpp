/**************************************************************************************************
 * 【题目描述】
 * 给定一个字符串 s 和 t，判断 s 是否 为 t 的子序列。这里的子序列 s 不一定是 t 中一段连续字符，比如 "ace"
 * 是 "abcde" 的一个子序列。
 * 
 *
 * 假定两个字符串都由小写字母组成。
 ...
 * 【解题思路】
 * 方法一：
 * 双指针法，两个指针初始分别指向两个字符串首字符，如果指向的字符相同，则指针都向右移一位，否则只将指向字
 * 符串 t 的指针右移一位，当任意一个字符串遍历完成时匹配截止。此时指向字符串 s 的指针如果指完了，就代表 s
 * 是 t 的子序列，否则代表 s 中还有字符未完成匹配。
 *
 * 此方法不限于题中所说的小写字母。
 * 
 * 
 * 方法二：
 * 假如字符串 t 很久才有变化，当输入非常多不同的 s 时，需要依次检查它们是否为 t 的子序列，这样会重复对 t 
 * 进行很多次遍历。可以牺牲一定的空间复杂度，记录字符串 t 中各个字符的位置信息，然后采用二分法来处理。初
 * 始时需要构建字符串 t 中字符的位置关系，可利用一个无序map，以 t 中各个字符作为key，对应的值为该字符在
 * 字符串中的下标数组，因为是遍历记录的，故数组中的下标是升序的。在遍历字符串 s 之前，需要记录下一次的搜
 * 索下标 searchIdx，即为在字符串 t 中匹配到的下标+1，然后遍历字符串 s，找到遍历字符对应在map下标数组中
 * 第一个比 searchIdx 大的下标，利用二分法查找有序序列元素。
 *
 * 此方法不限于题中所说的小写字母。
 * 
 * 
 ...
 * 就可以进行跳跃式的遍历了。比如：遍历到 t[0] 时，可知此时 s[0] 对应的字符在 t 中首次出现的下标是5，那
 * 么下次遍历就直接从 t[5+1] 开始，跳过了中间部分不需要处理的元素，从而提升效率，这种方式对超大量输入的s 
 * 字符串效率很高。因为遍历到 t 中某个字符 t[searchIdx] 时，能知道从searchIdx之后所有 t 中字符的下标位置，
 * 所以searchIdx值越小就知道 t 中字符下标信息越多，可以理解为规模越大，故此时小规模问题到大规模问题在 t 
 * 中是从后往前开始的，是一个倒过来动态规划的过程。
 *
 * 动态规划首先要建立状态转移方程，规定 f[searchIdx][ch] 是 t 中从searchIdx开始往后，字符ch首次出现的位
 * 置，如果 f[searchIdx][ch] 所表示的下标位置无意义的话，那ch的位置信息就可能在 f[searchIdx + 1][ch] 中
 * 出现；故有状态转移方程：
 * 
 * f[searchIdx][ch] == searchIdx               ①
 * f[searchIdx][ch] == f[searchIdx + 1][ch]    ②
 *
 * 注解：等式 ① 即表示 t 中sercharIdx所在位置的字符就是ch；等式 ② 则表示 t 中sercharIdx所在位置字符不是
 * ch，那ch首次出现的位置必然在 searchIdx + 1 之后；
 *
 * 大致过程：t 从后往前开始，遍历的 searchIdx 对应一个长度为26的数组映射26个小写字母，表示从 searchIdx 
 * 开始，后面的每个字符在 t 中首次出现的位置，因为是从后往前遍历的，所以只需要设置 t[searchIdx] 的位置，
 * 其余字符位置直接用 t[searchIdx + 1] 对应的值设置即可，对于不存在的字符的下标，默认以 t 的长度作为值，
 * 经过动态规划后可创建一个二维数组 idxArr，长度为 t 的长度+1（+1是为了建立最初时最小规模的问题，后续规
 * 模更大的问题就是基于这个最小问题开始求解的），每个元素对应一个26元素的数组映射26个小写字母在 t 中的位
 * 置信息。在动态规划创建好 t 的字符位置关系后，就可以开始遍历 s 了，如果当前遍历到 s 中的字符，在idxArr
 * 存在有意义的下标位置，则下次遍历可以直接从该下标位置+1处开始跳跃式遍历，当遇到在 idxArr 中的下标位置无
 * 意义时，表示从此时的搜索下标开始，s 对应的字符在 t 中已经不存在首次出现的位置了，即在 t 中该字符不存在，
 * 判定为非子序列。
 * 
 * 此方法按题意，仅限于小写字母。
 *
