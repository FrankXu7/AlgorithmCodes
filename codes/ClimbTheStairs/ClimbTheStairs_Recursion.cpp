 ...
 *
 * 【解题思路】
 * 这个问题的解，依赖于问题的规模，即 n 值越大，爬到第 n 阶的方法越多，且没有一个显式的表达式定义解；该问
 * 题中，爬到第 n 阶，则上一步必定为 n-1 或 n-2 阶，因为每次只能跨一阶或两阶；这其实就是一个斐波那契数列，
 * 利用递归求解；需要特别注意的是，爬楼方法数量随着 n 值会增长的越来越快，需要注意变量边界问题。
 * 
 * 【解题分析】
 * 时间复杂度：O(N^2)，问题规模n增加时，执行时间会显著增加，动态规划才是正解。
 * 空间复杂度：O(N)
 *
 * @author FrankX
 * @date 2021-04-15
