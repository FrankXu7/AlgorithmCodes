 ...
 *
 * 【解题思路】
 * 这个问题的解，依赖于问题的规模，即 n 值越大，爬到第 n 阶的方法越多，且没有一个显式的表达式定义解；可利
 * 用动态规划算法求解，该问题中，爬到第 n 阶，则上一步必定为 n-1 或 n-2 阶，因为每次只能跨一阶或两阶；故
 * 可以得到一个状态转移方程：f(n) = f(n-1) + f(n-2)，这其实就是一个斐波那契数列。需要特别注意的是，爬楼
 * 方法数量随着 n 值会增长的越来越快，需要注意变量边界问题。算法的时间复杂度为O(N)，空间复杂度依据不同的
 * 需求会有所不同：
...
unsigned int ClimbTheStairs_1(const unsigned int &n)
{
	if (n <= 2)
		...
		return 0; // n>=47 时，解会越出 unsigned int 边界
