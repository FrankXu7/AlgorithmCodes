/**************************************************************************************************
 * 【题目描述】
 * 给定一个取值范围为 [0, N] 的数组 nums，其长度为 N，数组中缺少一个数字，且数组中数值唯一，找到那个缺失的数字。
 * 实际上，就是在一个从0到N的自然数序列中，找到缺失的那个数字。
 *
 * 【输入】
 ...
 * 【解题思路】
 * 方法一：
 * 可以理解为一个从0到N的自然数序列，缺失了一个数字，所以可先将nums数组是按升序排序一下，那么数组元素
 * 两两差值为1，遍历到相邻元素差值大于1（实际上只可能为2）的情况，则可以找出缺失的数字。
 * 
 * 方法二：
 * 通过数据结构set集合来找，先将数组元素一次放入一个set集合，然后从0开始遍历到N，注意遍历的边界包含N，
 * 将遍历索引作为数据在set集合中查找存在性，找到不存在于set集合中的索引，即为缺失的那个数字。
 * 
 * 方法三：
 * 从0到N的自然数可以看成是一个等差为1的等差数列，可对该等差数列求和，然后对数组中元素求和，做个减法，
 * 减出来的差值即为缺失的那个数字。
 * 
 * 方法四：
 * 可以采用异或运算，根据相同的两个数进行异或计算结果为0，可将0到N的自然数序列与nums数组元素
 * 依次进行异或计算，并将结果异或赋值到一个变量，因为异或运算满足交换律，所以nums即使无序也没关系，
 * 在计算过程中，0到N的自然数序列，有一个数字会因为在nums中找不到和它相等的元素而剩下来，这个元素
 * 即为那个缺失的元素。
 * 这个方法需要特别注意的是，对nums数组遍历计算过后，因为nums缺少一个元素，所以遍历不能完全覆盖
 * 从0到N的所有自然数，会剩余最后一个数字N，这个N在数值上刚好等于nums数组的长度值。
 * 
 * 【解题分析】
 * 方法一：
