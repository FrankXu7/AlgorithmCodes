/**************************************************************************************************
 * 【题目描述】
 * 给定一个链表表头head，判断该链表是否为回文链表，是则返回true，否则返回false；回文链表即以中间节点分割，
 * 左右两侧数据对称相等。
 * 
 ...
 * 【解题思路】
 * 方法一：
 * 题意要求数值的取值区间是 [0, 9]，且节点个数区间为 [1, 10^5]，所以可直接遍历链表，将链表各节点组合为一
 * 个数字，然后判断这个数字是否为回文数即可。
 * 
 * 方法二：
 ...
 * 实际比方法二的效率更加低下。
 * 
 * 可以使用 “快慢指针” 法，慢指针步长为1，快指针步长为2，这样，当快指针指向链表尾部时，慢指针刚好指向链表
 * 中间，以慢指针为分割节点，将链表右侧进行一次反转，然后将分割节点左侧的链表，和分割节点右侧反转过后的链
 * 表，进行比较判断是否为回文链表。这里有一些注意点：
 * （1）慢指针指向分割节点时，其next需要指向null，从结构上彻底分割；
 * （2）反转链表时，表头节点首次赋值需要将next指向null，因首个表头最后会作为表尾，不置null会产生循环链表；
 * （3）记得要将分割节点右侧链表反转回来，并且重新续上，算法最后不会改变链表的结构。
 * 
 * 该方法的时间复杂度虽然是O(N)级别，但实际包含三个部分：遍历链表找到分割点O(N) + 反转链表节点O(N/2) +  
 * 比较链表节点O(N/2) + 二次反转链表节点并续上O(N/2)；牺牲了一定的时间复杂度，并且降低代码可读性，最终得
 * 到常数级别的空间复杂度。
 * 
 * 【解题分析】
 * 方法一：
 ...
 * 空间复杂度：O(N)
 * 方法三：
 * 时间复杂度：O(N) 实际复杂度为O(N + N/2 + N/2 +N/2)，实际以牺牲时间复杂度和代码可读性来降低空间复杂度
 * 空间复杂度：O(1)
 *
 * @author FrankX
 * @date 2021-09-07
