 ...
 * 【解题思路】
 * 方法一：
 * 由题意可知，前4位代表小时数，最大值为11，即为 1011，后6位代表分钟数，最大值为59，即为 111011，故时间数值上
 * 的最大值（11:59）为 1011 1110111，由此可知给定的数若其二进制中有超过8个数字1，则为非法数值，排除之。
 * 
 * 在次基础上，可利用一个两层循环，外层遍历所有的小时数 [0, 11]，内层遍历所有的分钟数 [0, 59]，当遍历确定
 * 一个小时和分钟数时，判断这两个数字二进制数中数字1的个数之和是否为给定的亮灯数量，是则将其推入最终数组反汇之。
 * 
 * 方法二：
 * 根据方法一的分析，先将给定数字二进制数中数字1的数量超过8个的排除。按照题意，其实可以将所有的可能性枚举
 * 出来，可以避免每次都对所有的可能性进行一一比较，建立两个无序map，分别用于小时数和分钟数，key用以标记
 * 对应的小时或分钟的数值二进制中有几个数字一，value则存储对应的所有可能性结果。
 * 
 * 然后按照小时数二进制中数字1的个数从[0, 3]遍历（当然这里肯定也要小于等于给定的亮灯数量），既然小时数占用的
 * 数字1个数确定，那分钟数占用的数字1的个数也得以确认。分别去查找之前得出的无序map中是否存在对应结果，若存在，
 * 则利用双层遍历得出它们所有的组合序列，返回之。
 * 
 * 
 * 【解题分析】
 * 方法一：
