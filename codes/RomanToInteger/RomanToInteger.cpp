 ...
 * }
 *
 * 补充：罗马数字按题意最大只能表示到4999，即 MMMMCMXCIX，从5000开始，字母表示会在【字母上加一横线】，
 * 打不出来，加在右上方：
 * 5000    V￣
 * 10000   X￣
 * 50000   L￣
 ...
 *
 * 【解题思路】
 * 遍历字符串，判断当前字符是否存在于数字映射map中，是的话将对应的值加到结果值。
 * 这里需要特别注意的是那六种特殊情况，特殊是因为一般罗马字母数字大的在数字小的左侧，而这六种情况则相反，
 * 数字小的在数字大的左侧，如：X(10)C(100)，C(100)M(1000)，可以得出一个规律：六种特殊情况的组合，
 * 都是“左小右大”的情况，不包含字母I，最重要的是，组合的值等于数字大的减去数字小的，例如：
 * XC = 90 = C(100) - X(10)
 * CM = 900 = M(1000) - C(100)
