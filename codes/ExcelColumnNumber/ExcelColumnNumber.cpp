 ...
 *
 * 【解题思路】
 * 最先想到的是按照进制转换的规则，直接求解：按照从低位到高位的的规则，将每一位数字的权值作为进制数字26的
 * 幂，然后每一位数字乘以进制26的对应权值的幂，然后求和。但进一步观察会发现，Excel列序号，不包含数字0，即
 * 没有0这个概念。而一般所说的进制转换计算，即普通的对应进制的四则计算，肯定是包含0这个概念的，所以为了保
 * 证列名是从1开始，即最小列名是‘A’，则需要在当前字符减去字符‘A’后再加上1，而在代码实现上，拆出这部分一样
 * 的逻辑，所以看起来是减1
 *
 * 【解题分析】
 * 时间复杂度：O(N)
