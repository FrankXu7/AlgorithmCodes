 ...
 *		I.	MostRightNode.right 为空，则将 MostRightNode.right = CurNode，这里保证了后续的当前遍历节点右移的时候，
 *			能够回到根节点，本质为借用叶子节点 MostRightNode 的右指针，该指针必为空值。然后 CurNode = CurNode.left，
 *			因为题目要求为前序遍历，所以此步骤遍历的 CurNode 为首次访问，将其推入数组，满足前序遍历“根左右”的特性。
 *		II.	MostRightNode.right 不为空且等于 CurNode，此时遍历的 CurNode 其实不是首次访问，
