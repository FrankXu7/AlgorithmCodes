 ...
 * 【解题思路】
 * 方法一:
 * 最先最直观想到的，就是遍历N以内的正整数了，然后对每一个正整数，遍历小于它的正整数，依次作整除的判断，
 * 统计之。想想就明白时间复杂度会很高，但通过质数的性质可知，0和1既不是质数也不是合数，所以可以直接从2开
 * 始遍历，又因为所有偶数中，只有2才是质数，所以可以从3开始遍历，遍历步长为2，排除所有偶数，提升效率。但
 * 还有改进空间，排除偶数后，还是会遍历到接近N的数，但仔细想想，两个正整数 a,b，如果满足 a*b=N，则必有以
 * sqrt(N) 为界，a,b 一大一小，分别位于 sqrt(N) 左右两侧，如果存在正整数 a,b 满足 a*b=N，只需要从小到大
 * 遍历到 sqrt(N) 即可，若存在因数，则另一个因数必大于等于 sqrt(N)。
 *
 * 方法二：
 * 可采用埃氏筛（埃拉托斯特尼筛法），如果一个数 NUM，被判断为是一个质数，显然，NUM 的 N(N>=2) 倍，必为一
 * 个合数。所以，可以用一个数组，以其下标作为操作数，当某一个下标所表示的数值被判断为是一个质数，可用该下
 * 标对应的数组元素值标记之，同时，可标记下标 NUM 后续 N(N>=2) 倍的所有下标，它们必然是合数。根据质数性
 * 质，除了2以外，所有的质数必为奇数，所以其实不需要处理下标为偶数的元素，但因为下标本身就是操作数，相当
 * 于数组有一半的空间是空闲的。这里标记的遍历可以进一步优化，若一个数值 NUM 被判断为质数，不必从NUM*2 开
 * 始标记数组元素为合数，因为它们肯定被 NUM 之前的质数标记过了，直接从 NUM*NUM 开始标记，遍历的每次步长
 * 为 NUM，依次标记对应下标数值为合数。最后，仔细考虑，每次的步长 NUM 会有什么效果，比如质数3，从 3*3 开
 * 始标记，步长即为3，则下一个处理的数是 9+3=12，显然，这是一个偶数，因为开头就排除了偶数2，所以剩余偶数
 * 中不可能存在质数了，这样就是冗余处理了，可再进一步优化，将步长设定为 NUM*2，因为 NUM 必然是一个奇数，
 * 而 NUM*2 必然是一个偶数，所以它们的和，也必然是奇数，这样就不会冗余的去处理偶数了。
 *
 * 方法三：
 * 线性筛（欧拉筛），在埃氏筛中，即使从 NUM*NUM 开始标记，仍会存在重复标记，比如45，当遍历到3时，会从3*3
 * 开始标记，故 3*15 会被标记为合数，再遍历到5，会从 5*5 开始标记，会将已经标记为合数的45会被 5*9 重复标
 * 记一次。合数有一个性质：任何合数都能分解为唯一一组多个质数的乘积。而线性筛的核心思想就是保证每个被筛选
 * 掉的合数，必定是被其最小质因数筛选掉的，此时中止继续筛选，就可以避免被重复标记了。所以，当遍历到某个num
 * 时，只需要将质数容器中的元素与 num 相乘，乘积必定为合数，标记之。而如果 num 是合数，则在质数容器中必定
 * 存在唯一一组质数的乘积等于 num，因为质数是按升序加入容器的，所以容器内的元素一定小于等于 num，此时，如
 * 果找到了第一个能够被 num 整除的质数，那么，这个质数就是 num 的最小质因数。在这里中止掉后续的筛选，避免
 * 重复标记。
 * 
 * 
 * 【解题分析】
