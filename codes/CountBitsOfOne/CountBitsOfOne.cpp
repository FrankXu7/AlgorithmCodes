 ...
 * 
 * 方法三：
 * 将1和另一个二进制位进行按位&运算，可以得出另一个二进制是0还是1，所以将1从目标数字二进制的最低位开始，
 * 依次左移并和目标数字进行按位&运算，统计目标数字二进制数中1的个数。
 * 
 * 方法四：
 * 目标数字和目标数字减去1，进行按位&运算：
 * （1）当目标数字二进制最低位刚好是1的时候，按位&运算之后，结果相当于比目标数字减少1；
 * （2）如果目标数字二进制最低位是0，则需要借位相减，原目标数字和减1后的目标数字，有部分二进制位不同，
 *		这部分不同的二进制位按位&运算的结果为0，这样就避免了统计到目标数字在借位减1时额外出现的数字1；
 * 统计到目标数字减到0为止。
 * 

 * 【解题分析】
 * 方法一：
 * 时间复杂度：O(1) 本质上是O(logN)，以2为低，题意固定32位，所以时间复杂度最大为5，是常数时间复杂度
 * 空间复杂度：O(1)
 * 方法二：
 ...
 * 空间复杂度：O(1)
 * 方法三：
 * 时间复杂度：O(1) 实际上为32，需要对每一位都进行依次计算以判断二进制位是否是1
 * 空间复杂度：O(1)
 *
 * @author FrankX
 * @date 2021-08-17
