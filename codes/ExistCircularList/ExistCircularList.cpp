 ...
 * 【解题思路】
 * 方法一：
 * 首先会想到给遍历的节点加上一个遍历过的标记，所以很容易有一个方案，将遍历过的节点地址存入一个map，当遍
 * 历到后续节点时，检查后续节点是否已经在map中存在。存在标识有环路，否则无环路。
 *
 * 方法二：
 * 标记每一个被遍历过的节点，则空间复杂度不是常量。可以考虑快慢指针，慢指针每次遍历前进1位，快指针每次前
 * 进2位。如果存在环路的话，快指针迟早会追到慢指针，即快慢指针地址值相同。否则快指针会更先一步遇到下一个
 * 指向为空的节点。
 * 
 * 【解题分析】
 * 方法一：
