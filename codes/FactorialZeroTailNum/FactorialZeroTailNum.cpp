 ...
 *
 * 【解题思路】
 * 任意数字的乘积，最终都能转化成0-9单个数字的乘积，而对0-9单个数字来说，两个单个数字相乘，结果中尾数要存
 * 在0，则必然存在2*5的情况，比如8*5=4*2*5=40，对于阶乘的定义来说，数字2自然是不会缺少的，比如16就能分解
 * 为4个2的乘积，而数字5相对来说少很多，所以，针对能够产生0的必然组合2*5来说，找到阶乘中所有5的个数，即为
 * 最终阶乘结果尾数中0的个数。
 * 
 * 对于任意的连续5个数字，必定有至少1个2*5的组合会为最终结果贡献一个尾数0，可以将需要阶乘的数除以5，将数
 * 字按5分成N份，每一份至少都有一个2*5组合，所以有N个0尾数。如果N是一个大于等于5的数字，则要继续递归的除
 * 以5，并累加所除的结果。因为，对小于10以内的数字来说，连续5个数字只有1个2*5的组合，但是对于十位数，百位
 * 数以上的数字，任意连续5个数字，可能有多个2*5的组合，比如[21-25]这5个数字，22*25已经贡献了一组2*5，即
 * 11*2*5*5，但还剩一个5，所以[21-25]贡献了两组2*5，综上，大致过程为：
