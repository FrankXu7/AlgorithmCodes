 ...
 * 【解题思路】
 * 方法一：
 * 对于一个二进制数来说，如果是奇数，那最低位的数字肯定是1，若是偶数则为0，所以，可以对整数进行迭代对2取
 * 余，取余结果非0即1，统计其中数字1的数量，在对2取余后，最低位已经处理过了，通过除2排除之，直至将整数除
 * 至0。
 * 
 * 方法二：
 * 除了可以通过对2取余判断二进制最低位之外，还可以将一个整数和数字1按位&运算，如果最低位是1，则运算结果是
 * 1，否则就是0，统计其中结果是1的次数，和方法一类似，最低位处理完后，通过向右移位操作来排除最低位，直至
 * 整数为0。
 * 
 * 方法三：
 * 窗口算法，将数字1，每次左移1位，因为数字1不论左移多少位，整个二进制数中都只包含有一个数字1，将其与整数
 * 按位&运算，所得结果为1，则表示整数对应的二进制位也是数字1，左移是有上限的，对于 int 类型来说，最多只有
 * 32位，所以最多只需要左移32次即可。
 * 
 * 方法四:
 * Brian Kernighan 算法：表达式 N & (N-1) 是将一个整数N的二进制位最后一个1消除。依据该算法思想，使用 N &
 * (N-1) 作为核心算子，将每次计算结果赋值回整数N，这样每次计算后都能将整数的二进制数中最低位的数字1消除，
 * 直至整数为0。
 * 
 * 方法五：
 * 采用动态规划思想，观察连续自然数二进制规律，可知：一个二进制数N，右移1位，等价于除2，相当于移除了二进
 * 制数N的最低位，此时 N>>1 中数字1的个数，即为二进制数N中数字1的个数，加上被移除的最低位中数字1的个数，
 * 可以看出，二进制数N的数字1的个数，依赖于 (N>>1) 的二进制数中数字1的个数，更大问题的解决方案依赖于更小
 * 问题，答案依赖于问题的规模，可以用动态规划算法求解。因为 N>>1 的值肯定小于N，而 N>>1 在这之前就已经计
 * 算并保存到数组中了，可以直接获取，此时还需要知道在 N>>1 后排除掉的二进制最低位是1还是0，可以通过对 (N
 *  & 1) 来判断奇数与否来确定二进制最低位，奇数的二进制最低位是1，偶数是0，综上，可建立动态规划算法的状态
 * 转移方程：countArr[N] = countArr[N >> 1] + (N & 1); 其中，countArr[N >> 1] 即为 N 在排除二进制最低
 * 位后数字1的个数，由之前保存的结果求得，而 (N & 1) 即为被排除掉的最低位，如果是1，则统计之，否则为0对结
 * 果也不会有影响。
 * 
 * 【解题分析】
 * 方法一：
