 ...
 *
 * 【解题思路】
 * 可以发现规律：在二叉树中，从树根结点到某节点的路径一定是唯一的。考虑用两个队列，一个存结点，一个存路径
 * 上结点之和，大致过程如下：
 * （1）出队一个结点，同时出队一个值，这个值就是从树根结点到出队的结点的路径上结点之和，如果这个出队结点
 *      是叶节点，且对应的值等于目标值，即得出最终结果，可直接截断返回；
 * （2）否则，将出队结点的左右子结点入队，将结点的值入队时，需要加上出队结点的值之后，再入队，这样就相当
 *      于逐层计算了每层的每个结点，从树根结点到当前结点路径上节点之和；
 *
 * 【解题分析】
 * 时间复杂度：O(N)
 * 空间复杂度：O(N)
