 ...
 * 符的数量 left 和 right，有一个客观事实是，如果以左括号字符开头，当 curStr 长度达到 num*2 且其中的左右
 * 括号字符数量都相等的情况下，那当前的括号组合 curStr 肯定是有效的。
 *
 ...
 *
 * 方法三：
 * 对于任何一个有效的括号组合，都可以看做是字符串 '(' + leftStr + ')' + rightStr 构成的，其中的 leftStr
 ...
 * 构造括号的对数为 num 时的有效组合数量。其中，假设 leftStr 中的括号的对数为 idx 时，对应 rightStr 中的
 * 括号对数就是：num - idx - 1，综上，更大规模的问题可由更小规模问题解决，可以用动态规划的方法解题。
 * 
 ...
 *                         还有两个嵌套的for循环用以生成当前括号的对数 curNum 的所有有效的括号组合，这
 *                         两个循环的复杂度在[2^N, 3^N]的区间内，更加靠近2^N，且随着N越大，会远小于3^N
