 ...
 *
 * 方法二：
 * 对暴力法进行优化，将 '(' 或 ')' 添加到 curStr 时，只在仍然保持括号组合有效的情况下添加。在一个有效的括号
 * 组合中，左右括号字符的数量一定是相等的。所以在递归回溯的过程中，可以维护 curStr 字符串中左右括号字符的数量
 * left 和 right，有一个客观事实是，如果以左括号字符开头，当 curStr 长度达到 num*2 且其中的左右括号字符数量
 * 都相等的情况下，那当前的括号组合 curStr 肯定是有效的。
 *
 * 在递归调用中，承接方法一的逻辑，只不过在添加左括号字符时，left<num 必须成立才执行添加，同理，紧接着在添加
 * 右括号字符时，必须满足 right<left 才添加。
 *
 * 对于有效性判断，因为初始递归调用从 '(' 字符开始处理，故不会出现以右括号字符开头的情况，同时，两个分支的判
 * 断也保证了 curStr 长度达到 num*2 时左右括号字符数量相等，则此时 curStr 必为有效的括号组合。
 *
 * 方法三：
 * 对于任何一个有效的括号组合，都可以看做是字符串 '(' + leftStr + ')' + rightStr 构成的，其中的 leftStr 和
 * rightStr 都分别是一个有效的括号组合（可以为空），且 leftStr 和 rightStr 中的括号的对数等于 num-1，通过枚
 * 举 leftStr 和 rightStr 对应的所有情况，就可以用已求得的比 num 对数更少的有效括号组合的数量，来构造括号的
 * 对数为 num 时的有效组合数量。其中，假设 leftStr 中的括号的对数为 idx 时，对应 rightStr 中的括号对数就是：
 * num - idx - 1，综上，更大规模的问题可由更小规模问题解决，可以用动态规划的方法解题。
 * 
 * 构建状态转移方程，假设括号的对数为 num 时的所有有效组合的集合表示为 S[num]，则：
 * S[num] = { '(' + leftStr + ')' + rightStr | leftStr 为枚举 [0, num-1] 时的所有有效括号组合的数量，对
 * 应的，rightStr 为枚举 [num-idx-1, num-1] 时的所有有效括号组合的数量 }
 * 
 * 【解题分析】
 * 方法一：
 ...
 * 空间复杂度：O(2N) 如果每一层递归的视作O(1)，那么复杂度为 O(2N)
 * 方法三：
 * 时间复杂度：O(N^2 * 2^N) 外层两层循环可以确定每层的循环上限最差的情况和 num 的值相等，在内层循环中，还有
 *                         两个嵌套的for循环用以生成当前括号的对数 curNum 的所有有效的括号组合，这两个循环
 *                         的复杂度在 [2^N, 3^N] 的区间内，更加靠近 2^N，且随着 N 越大，会远小于 3^N
