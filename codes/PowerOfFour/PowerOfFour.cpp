 ...
 *
 * 方法二：
 * 采用窗口算法，构造一个4的幂。对于4的幂的二进制数中，只会在最高位存在一个数字1，所以可以从1开始，每次左
 * 移两位，相当于乘上4，等价于给幂次加1，然后将这个构造的数和给定整数进行按位&运算。如果给定的数是4的幂，
 * 那么按位&运算后结果不变，否则给定整数的二进制数中，除了与构造的二进制数对应位置的数字1不变之外，其余位
 * 置的二进制位都会因为和0进行按位&运算而变成0，运算结果不等于原给定整数。
 * 
 * 方法三：
 ...
 * 所以，可以构造一个奇数位都是1的二进制数，如果给定整数是4的幂，那么与这个构造的二进制数按位&运算结果必
 * 为0，否则就不是4的幂了，该构造的二进制数可以写成：
 * 10101010 10101010 10101010 10101010 10101010 10101010 10101010 10101010
 * 写成十六进制即为：0xAAAA'AAAA'AAAA'AAAA
 * 
 * 需要特别注意的是，构造的数本质上是一个遮罩，如果一个不是4的幂能透过这个遮罩，容易被误判，比如数字5，二
 * 进制是101，但它不是4的幂。因为4本身就是2的2次幂，且4的幂也必然是2的幂，所以可以先判断是否为2的幂，然后
 * 再用构造的遮罩数过滤。
 * 
 * 方法四：
 * 4的幂表示为：4^N，等价于 (3+1)^N，对该表达式进行二项式展开（二项式展开有公式），展开后，最后一项是1，
 * 前N-1项中，至少包含一个因子3，所以最终展开结果可写成如下形式：3(展开结果排除最后一项提取公因式3) + 1，
 * 可以看出如果给定整数是4的幂，那么将其对3取模，结果必为1，同样需要注意的是有些非4的幂对3取模结果也是1，
