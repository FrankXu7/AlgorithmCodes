 ...
 *
 * 【解题思路】
 * 因为回文数具有对称性，翻转回文数后半部分的数字，会等于前半部分，
 * 所以依次剥离其个位数（对10取余），并从个位开始添加到一个新数字当中，
 * 当指定的数字减小到小于等于新的数字后，将相当于将该整数对半分开了，
 * 如果两个对半分开的数字相等，则原整数是回文数。
 * 需要特别注意的是，奇数位的整数，正中间的数字和它自身对称，如：12321 也是回文数。
 * 
 * 【解题分析】
 * 时间复杂度：O(N)
 * 空间复杂度：O(1)
