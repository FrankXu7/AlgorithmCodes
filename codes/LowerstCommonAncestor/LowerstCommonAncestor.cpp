/**************************************************************************************************
 * 【题目描述】
 * 给定一颗二叉搜索树，找出给定的两个节点的最近公共祖先节点。
 * 规定所有节点的数值都是唯一的，两个节点为不同节点。
 * 
 ...
 * 越晚入队和出队，当遇到两个出队节点不同时，上一次出队的相同节点即为它们的最近公共祖先。
 * 在找节点路径的遍历中，因为是BST树，所以查找节点小于当前遍历节点时，则向左遍历，否则向右遍历。
 * 
 * 方法二：
 * 因为两个节点的路径都是从根节点出发，所以可以从根节点开始，就通过BFS遍历一层一层比较，这样只需要一次循环，
 * 且不需要额外的辅助空间，当遍历到某个节点时，会遇到以下情况：
 * （1）如果给定的俩节点都小于当前遍历节点，那它们的公共祖先肯定在左侧分支上，继续往左侧查找；
 * （2）反之，如果俩节点都大于当前遍历节点，那它们的公共祖先必然在右侧分支上，则向右侧查找；
 * （3）不是上面两种情况，那意味着给的俩节点以当前遍历节点为界，分散在了两侧分支（也包含其中某个节点就是当前
 *		遍历的节点这种情况），这就表示了当前遍历的节点就是他俩的最近公共祖先了。
 * 
 * 
 * 【解题分析】
 * 方法一：
 * 时间复杂度：O(N) 实际上要经历三次循环，查找两条路径各需循环一次，比较路径需要循环一次，
 *					它们的平均复杂度都是O(logN)，故平均复杂度之和为log(3*logN)，最差情况为单链表O(3N)
 * 空间复杂度：O(N) 需要存储两条路径节点，复杂度都是O(logN)，平均复杂度之和为O(2*logN)，最差情况为单链表O(2N)
 * 方法二：
