 ...
 * 相同时，将map统计的对应字符数量自减（即使没有也不影响，自减为负数，表示t中的字符在s中不存在），如果两 
 * 个字符串是异位词，那map中各个字符的统计数量是0，只要遇到字符数量不为0，可直接返回false；
 * 
 * 方法二：
 * 按照异位词的定义，如果两个字符串是异位词，那么在排序后两个字符串应该相等。所以可以先对两个字符串来一波
 * 排序，然后比较字符串是否相等，相等表示是异位词，返回true；
 * 
 * 这两个方法需要注意的是，优先判断两个字符长度，长度不同，必然不是异位词。
 * 
 * 【解题分析】
 * 方法一：
 * 时间复杂度：O(N) 实际为O(3N)，统计s中的字符数量，比较自减s和t相同的字符数量，检查map中字符统计数量
 * 空间复杂度：O(N) 最差的情况就是字符串中每个字符唯一，需要O(N)的空间复杂度 
 * 方法二：
 * 时间复杂度：O(NlogN) 采用了语言自实现的排序算法，实际为O(logN + N)，排序为O(logN)，比较字符串为O(N) 
