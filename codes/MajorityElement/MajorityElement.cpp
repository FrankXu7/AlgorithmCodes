/**************************************************************************************************
 * 【题目描述】
 * 给定一个含有n个元素的数组，找到其中的多数元素，所谓多数元素即出现次数 大于n/2 的元素。这里题设数组必定
 * 非空，且一定存在多数元素。
 * 
 * 
 * 【输入】
 ...
 * 【解题思路】
 * 方法一：
 * 最先直接想到的方法，是牺牲空间复杂度来统计各个元素的出现次数。用一个map来记录元素出现次数，map的键为元
 * 素值，值为该元素出现的次数，同时用一个临时变量记录遍历到当前为止的出现元素最多的值。需要注意的是，算法
 * 逻辑本身是找到出现次数最多的元素，并没有验证多出元素的出现次数是大于n/2的，因为题设中假定非空数组一定
 * 存在多数元素。
 * 
 * 方法二：
 ...
 * 
 * 方法三：
 * 利用摩尔投票法，将两两相邻的一对不相等的数据剔除，因为多数元素个数大于 n/2，所以最终剩余的投票数量大于
 * 0的元素，就是多数元素，如果最终没有元素的投票数量大于0，表示不存在个数超过 n/2 的元素。大致过程：
