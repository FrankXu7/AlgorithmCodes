/**************************************************************************************************
 * 【题目描述】
 * 判断一个无符号整数是否为快乐数。对于一个无符号整数n，将n的各位的平方根之和，赋值给n，得到一个新的无符
 * 号整数，重复这个过程，最终平方和要么为1，要么会在一组数之间永远循环。平方和最终为1时，最初的整数n即为
 * 快乐数。
 *
 * 【输入】
 * Input Number: 19
 ...
 *
 * 【解题思路】
 * 首先明确一点，任意一个无符号整数，其各位上的平方之和，以及递归计算下去的平方之和，都一定是小于等于原整
 * 数的位数都取9时的平方和：比如一个三位数 666，它的各位的平方和是108，继续求平方和是65，后面继续求的话，
 * 要么是快乐数平方和为1，要么在一组数之间循环，但它们都一定小于等于 999 各位的平方之和 243；
 * 
 * 方法一：
 * 针对思路所述的性质，可以将每次求得的各位上的平方之和，存在一个数据集合中，如果下一次求得的平方和存在于
 * 数据集合，则代表进入了一组数的循环，标识原整数不是快乐数，否则平方和会遇到等于1的情况。因为平方和要么
 * 等于1是快乐数，要么进入一组循环，所以即使原整数数值很大，平方和的数据集合也不会有数据量巨大的问题，比
 * 如 UINT_MAX = 4294967295，其平方和数据集合大小仅为17；
 * 
 * 方法二：
 * 在数学上，不快乐数有如下性质：不快乐数的计算最终都会进入到 4 → 16 → 37 → 58 → 89 → 145 → 42 → 20 → 4
 * 的循环中。所以针对方法一，可以将数据集合固定为 { 4, 16, 37, 58, 89, 145, 42, 20 } ，如果是不快乐数，
 * 则一定会有平方和的结果在这个数据集合中，否则就是快乐数。
 * 
 * 方法三：
