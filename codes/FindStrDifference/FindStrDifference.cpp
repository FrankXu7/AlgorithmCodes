/**************************************************************************************************
 * 【题目描述】
 * 给定两个字符串 s 和 t，其中字符串 t 由 字符串 s 随机重排，然后在字符串 t 中随机位置插入一个字符；
 * 找出在字符串 t 中新插入的字符；
 * 
 * 【输入】
 * s="abcde" t="dbafce"
 ...
 * 【解题思路】
 * 方法一：
 * 由题可知字符串 t 中会比 s 多一个字符，可以先用一个无序map统计字符串 s 中各字符数量，然后遍历字符串
 * t 和 s 中相同的字符，将无序map中对应字符的统计数量减1；当遇到不存在于字符串 s 中的字符，或统计数量为0时，
 ...
 * （3）一个数和0异或后结果不变；
 * 
 * 字符串 t 中除了新插入的那个字符之外，其余字符都和字符串 s 中相同，故可将两个字符串中所有字符都进行异或运算，
 * 相同的字符两两异或为0，最后剩下的值，就是字符串 t 中新插入的字符；
 * 
 * 方法三：
 * 因为字符串 t 中除了新插入的那个字符之外，其余字符都和字符串 s 中相同，故可以分别计算两个字符串各个字符相加之和，
 * 可知字符串 t 的字符之和必大于字符串 s 的字符之和，两和之差，即为字符串 t 中新插入的字符；
 * 
 * 
 * 【解题分析】
 * 方法一：
