/**************************************************************************************************
 * 【题目描述】
 * 有两个不存在环形的单链表，找出并返回这两个单链表相交的起始节点，如果没有交点则返回nullptr；这里需要注
 * 意的是，相同的节点，指的是同一块内存，如果仅仅是数据相同，不一定是同一个节点。
 *
 * 【输入】
 * A: A1 -> A2 -> A3 -> C1 -> C2
 * B: B1 -> B2 -------> C1 -> C2
 ...
 *
 * 【解题思路】
 * 为了能够有常数的空间复杂度，可以采用双指针（假设为pA和pB）的方式，两个指针同时从两个单链表（假设为A和B）
 * 开始遍历，且两个单链表节点长度相差 n(n>=0)，若链表A遍历完，则链表B肯定还剩 n 个节点还未遍历，此时，pA
 * 重新指向链表B的头节点进行遍历，这时，pA和pB都在链表B上，且相差 n 个节点。同理，当pB遍历完后，pB重新指
 * 向链表A的头节点进行遍历，如果两个单链表有交点，则在pA和pB遍历完切换到另一个链表进行遍历时，必定会相遇
 * 于相交节点。反之，则会无限的在链表A和B之间来回切换遍历，所以一定要标记切换的行为，有交点的情况下，双指
 * 针最多各进行一次切换能同时指向相交节点（相同长度时不用切换）。
 * 
 * 【解题分析】
