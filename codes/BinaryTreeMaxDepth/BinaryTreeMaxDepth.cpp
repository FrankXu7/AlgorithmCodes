 ...
 * 【解题思路】
 * 方法一：
 * 采用BFS遍历，利用两个队列交替存储彼此队列中存储节点的子节点，直到两个队列为空，每次遍历前，判断队列中
 * 有元素，则表示该层非空，深度+1，具体步骤：
 * （1）BFS一层节点入Queue1；
 ...
 * 
 * 方法二：
 * 依然采用BFS遍历，只利用一个队列，记录每一层的节点数量，出队时节点数量依次减一，并将该节点的非空的子节
 * 点入队，节点数量减为0时，深度+1，并且节点数量更新为下一层的数量，具体步骤：
 ...
 * 方法一：
 * 时间复杂度：O(N)
 * 空间复杂度：O(N) 实际最大为 N/2，因为每次只入队一层节点，完全二叉树最底层节点数量最大值即为N/2
 * 方法二：
 * 时间复杂度：O(N)
 * 空间复杂度：O(N) 实际最大为 N/2，因为每次只入队一层节点，完全二叉树最底层节点数量最大值即为N/2
 * 
...
unsigned int BinaryTreeMaxDepth_1(const TreeNode<int>* treeRoot)
{
...
unsigned int BinaryTreeMaxDepth_2(const TreeNode<int>* treeRoot)
{
	unsigned int treeDepth = 0;
...
int main()
{
	...
	cout << "\n[Solution 1] The Binary Tree Depth: " << BinaryTreeMaxDepth_1(treeRoot) << endl;
	cout << "[Solution 2] The Binary Tree Depth: " << BinaryTreeMaxDepth_2(treeRoot) << endl;
