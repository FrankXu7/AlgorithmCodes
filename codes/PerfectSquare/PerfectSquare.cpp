 ...
 * 【解题思路】
 * 方法一：
 * 依据两个乘数相同，可知总共就只有10种情况：0*0, 1*1, ... 8*8, 9*9，分析结果可知个位数必然是{ 0, 1, 4,
 * 5, 6, 9 }，故可将给定正整数NUM对10取余，若个位数为 { 2, 3, 7, 8 }，则必然不是完全平方数。然后依次遍历
 * 小于等于 NUM/2 的自然数，判断它们的平方是否等于给定的正整数NUM，这里需要注意，如果NUM是0或1，它们的完
 * 全平方数就是它们自己，可先排除之。NUM的数值越大，那么确定最终结果的值，和NUM/2的差值也就越大，所以，随
 * 着NUM的增大，遍历次数的增长小于NUM的增长速率。
 * 
 * 方法二：
 * 进一步优化方法一，在一个序列中（指一组自然数序列）找到给定的数，应该首先考虑二分法。在区间 [2, NUM/2]
 * 中，除了0和1的平方根是它们自身外，存在完全平方根的最小的数是4，它的完全平方根是4/2，在此之后的数的完全
 * 平方根必然小于NUM/2；
 * 
 * 方法三：
 * 牛顿迭代法：给定正整数 NUM 如果存在完全平方根 x，则必有 x^2 = NUM，故可将求完全平方根转换为求 f(x) = 
 * x^2 - NUM = 0 的根。过程为先任意取一点作为根，这里取 NUM/2，过 x = NUM/2 做曲线 x^2 的切线，求得交于
 * X轴的切点，将该切点的X轴坐标作为下一条切线的切点，重复此过程，切线逐渐向满足 x^2 - NUM = 0 的根逼近，
 * 当逼近到等式成立，则找到了NUM的完全平方根，如果有 x^2 < NUM，则表明NUM的完全平方根不存在。
 * 
 * 依据 f(x) = x^2 - NUM = 0 推导的牛顿迭代公式：x = (x + NUM/x) / 2，将x迭代带入此公式求出满足等式的根。
 ...
 * 
 * 方法四：
 * 总结规律，若已知某个数N存在平方根，则其平方根为 N^2，显然它下一个存在完全平方根的数为 (N+1)^2，所以，
 * 对于两两相邻的存在完全平方根的数而言，它们存在如下关系：(N+1)^2 - N^2 = 2N+1，即从0开始，接下来的一个
 * 存在完全平方根的数是从0开始逐渐加上对应奇数序列中的一个奇数，例如：
 * 0*0=0  +1
 * 1*1=1  +3
 * 2*2=4  +5
 * 3*3=9  +7
